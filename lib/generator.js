...
class NameGenerator {
  ...
  async generateNames(categorySlug, generatorKey, filters = {}, count = 10) {
    ...
    // Special handling for Alien generator - gender and format filters
    if (generatorKey === 'alien') {
      const gender = (filters.gender && filters.gender.trim()) || 'male';
      const format = (filters.format && filters.format.trim()) || 'full_name';
      
      // Get gender-specific first names
      let firstNames = [];
      if (gender === 'male') {
        firstNames = generator.data.male || [];
      } else if (gender === 'female') {
        firstNames = generator.data.female || [];
      } else if (gender === 'non_binary') {
        firstNames = generator.data.non_binary || [];
      }
      ...
      if (!firstNames || firstNames.length === 0) {
        throw new Error(`Invalid data structure for Alien generator. First names for gender '${gender}' not found.`);
      }
    ...
    // Special handling for Angel generator - gender and format filters
    if (generatorKey === 'angel') {
      const gender = (filters.gender && filters.gender.trim()) || 'male';
      const format = (filters.format && filters.format.trim()) || 'full_name';
      ...
      if (!firstNames || firstNames.length === 0) {
        throw new Error(`Invalid data structure for Angel generator. First names for gender '${gender}' not found.`);
      }
