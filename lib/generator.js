...
class NameGenerator {
  ...
  async generateNames(categorySlug, generatorKey, filters = {}, count = 10) {
    ...
    // Special handling for Vampire generator - gender-based with first name repetition prevention
    if (generatorKey === 'vampire') {
      ...
      if (!namePool || namePool.length === 0) {
        throw new Error(`Invalid data structure for vampire generator. Names not found for gender: ${gender}`);
      }
      ...
      // Generate names with first name repetition prevention
      const results = [];
      const usedFirstNames = new Set();
      let attempts = 0;
      const maxAttempts = shuffledPool.length * 3; // Prevent infinite loops
