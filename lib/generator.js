...
class NameGenerator {
  ...
  async generateNames(categorySlug, generatorKey, filters = {}, count = 10) {
    ...
    if (generatorKey === 'assassin') {
      ...
      const combinedPrefixes = generator.data.combined_prefixes || [];
      const combinedSuffixes = generator.data.combined_suffixes || [];
      
      if (!combinedPrefixes || combinedPrefixes.length === 0) {
        throw new Error('Invalid data structure for assassin generator. Combined prefixes not found.');
      }
      if (!combinedSuffixes || combinedSuffixes.length === 0) {
        throw new Error('Invalid data structure for assassin generator. Combined suffixes not found.');
      }
      ...
      if (!prefixShuffled || prefixIndex >= prefixShuffled.length) {
        prefixShuffled = this.shuffleArray([...combinedPrefixes]);
        this.namePools.set(prefixPoolKey, prefixShuffled);
        prefixIndex = 0;
      }
      if (!suffixShuffled || suffixIndex >= suffixShuffled.length) {
        suffixShuffled = this.shuffleArray([...combinedSuffixes]);
