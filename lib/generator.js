...
class NameGenerator {
  ...
  calculateSimilarityScore(currentGen, candidateGen, currentCategorySlug, candidateCategorySlug) {
    ...
    // Word matching (highest priority - 100 points per match)
    const matchingWords = currentWords.filter(word => candidateWords.includes(word));
    score += matchingWords.length * 100;
    
    // Partial word matches (50 points per partial match)
    for (const currentWord of currentWords) {
      for (const candidateWord of candidateWords) {
        if (currentWord.includes(candidateWord) || candidateWord.includes(currentWord)) {
          score += 50;
        }
      }
    }
  ...
  async getRelatedGenerators(categorySlug, generatorKey) {
    ...
    // Get top generators (max 6)
    let related = scoredGenerators.slice(0, 6);

    // If we have fewer than 3, fill with popular generators
    if (related.length < 3) {
