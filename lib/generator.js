...
class NameGenerator {
  ...
  async generateNames(categorySlug, generatorKey, filters = {}, count = 10) {
    ...
    // Special handling for vampire generator - combine first names with surnames (no format filter, just gender)
    if (generatorKey === 'vampire') {
      ...
      if (surnames.length === 0) {
        throw new Error('Surnames not available for vampire generator');
      }
      ...
      let surnameShuffled = this.namePools.get(surnamePoolKey);
      let surnameIndex = this.nameIndices.get(surnamePoolKey) || 0;
    ...
    // Special handling for brand generator - map category filter to data keys
    if (generatorKey === 'brand') {
      ...
      if (category === 'all_categories') {
        ...
        for (const key in generator.data) {
          if (Array.isArray(generator.data[key])) {
            namePool = namePool.concat(generator.data[key]);
          }
        }
      } else {
        // Map filter value to data key (e.g., "tech_innovation" -> "Tech_Innovation_Brands")
        const categoryKey = category.split('_').map(word => 
          word.charAt(0).toUpperCase() + word.slice(1)
        ).join('_') + '_Brands';
        
        if (generator.data[categoryKey] && Array.isArray(generator.data[categoryKey])) {
          namePool = generator.data[categoryKey];
        } else {
          throw new Error(`Category ${category} not found in brand generator data`);
        }
      }
    ...
    // Special handling for guild generator - combine prefixes with suffixes or use single aliases
    if (generatorKey === 'guild') {
      const style = filters.style || 'compound';
      const prefixes = generator.data.prefixes || [];
      const suffixes = generator.data.suffixes || [];
      const singleAliases = generator.data.single_aliases || [];
      
      if (style === 'simple') {
        // Use single aliases, 50% prefixed with "The"
        const poolKey = this.createPoolKey(categorySlug, generatorKey, filters);
        let shuffledPool = this.namePools.get(poolKey);
        let currentIndex = this.nameIndices.get(poolKey) || 0;
        
        if (!shuffledPool || currentIndex >= shuffledPool.length) {
          shuffledPool = this.shuffleArray([...singleAliases]);
          this.namePools.set(poolKey, shuffledPool);
          currentIndex = 0;
        }
        ...
        for (let i = 0; i < count; i++) {
          ...
          // 50% chance to prefix with "The"
          const name = (Math.random() < 0.5) ? `The ${alias}` : alias;
          results.push(name);
          currentIndex++;
        }
        ...
        // Compound: combine prefix with suffix, replacing [Name] with prefix
        const prefixPoolKey = `${categorySlug}:${generatorKey}:prefix`;
        const suffixPoolKey = `${categorySlug}:${generatorKey}:suffix`;
        ...
        for (let i = 0; i < count; i++) {
          ...
          const name = suffix.replace(/\[Name\]/g, prefix);
          results.push(name);
          prefixIndex++;
          suffixIndex++;
        }
    ...
    // Special handling for clan generator - replace [Name] in titles with themed names
    if (generatorKey === 'clan') {
      const type = filters.type || 'animal';
      const titles = generator.data.titles || [];
      const names = generator.data[type] || [];
      
      if (titles.length === 0 || names.length === 0) {
        throw new Error(`Invalid data structure for clan generator. Type: ${type}`);
      }
      
      const titlePoolKey = `${categorySlug}:${generatorKey}:title`;
      const namePoolKey = `${categorySlug}:${generatorKey}:name:${type}`;
      ...
      if (!nameShuffled || nameIndex >= nameShuffled.length) {
        nameShuffled = this.shuffleArray([...names]);
      ...
      for (let i = 0; i < count; i++) {
        ...
        const title = titleShuffled[titleIndex];
        const name = nameShuffled[nameIndex];
        const clanName = title.replace(/\[Name\]/g, name);
        results.push(clanName);
        titleIndex++;
        nameIndex++;
      }
    ...
    // Special handling for poison generator - format as "[name], [descriptor]"
    if (generatorKey === 'poison') {
      const baseNames = generator.data.baseNames || [];
      const descriptors = generator.data.descriptors || [];
      
      if (baseNames.length === 0 || descriptors.length === 0) {
        throw new Error('Invalid data structure for poison generator');
      }
      
      const basePoolKey = `${categorySlug}:${generatorKey}:base`;
      const descPoolKey = `${categorySlug}:${generatorKey}:desc`;
      ...
      for (let i = 0; i < count; i++) {
        ...
        const descriptor = descShuffled[descIndex];
        results.push(`${baseName}, ${descriptor}`);
        baseIndex++;
        descIndex++;
      }
